
namespace MultiPlug.Ext.Nuget.Controllers
{
    class Templates
    {
        internal const string SettingsNavigation = "NugetGalleryNavigation";
        internal const string SettingsHome = "NugetGalleryHome";
        internal const string SettingsSearch = "NugetGallerySearch";
        internal const string SettingsDownloads = "NugetGalleryDownloads";
        internal const string SettingsAbout = "NugetGalleryAbout";
    }
}
